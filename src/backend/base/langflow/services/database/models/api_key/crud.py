...
async def get_api_keys(session: Session, user_id: UUID) -> list[ApiKeyRead]:
...
async def create_api_key(session: Session, api_key_create: ApiKeyCreate, user_id: UUID) -> UnmaskedApiKeyRead:
...
async def delete_api_key(session: Session, api_key_id: UUID) -> None:
...
async def check_key(session: Session, api_key: str) -> User | None:
...
async def update_total_uses(api_key_id: UUID):
    ...
    with session_scope() as session:
        new_api_key = session.get(ApiKey, api_key_id)
